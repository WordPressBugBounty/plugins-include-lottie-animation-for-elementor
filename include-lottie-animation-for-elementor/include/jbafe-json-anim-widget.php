...
class Json_Based_Animation_Addon_Kap_Asias extends Widget_Base {
	...
	protected function render() {
		...
		if(!empty($settings['la_action'])){
			$la_action = esc_attr($settings['la_action']);
		}
			...
			$la_op .='<div class="jbafes-lotties-animation-wrapper-iconic jbafes-iconic'.esc_attr($id).'">';
				if(!empty($settings['la_lconic_layout_heading']) && $settings['la_lconic_layout_heading']=='yes'){
					if(!empty($settings['heading_text'])){
						$la_op .='<'.esc_attr($this->json_check_and_validate_html_tag($heading_tag)).' class="jbafes-iconic-heading">'.esc_attr($settings['heading_text']).'</'.esc_attr($this->json_check_and_validate_html_tag($heading_tag)).'>';
					}
				}
				if(!empty($settings['la_lconic_layout_title']) && $settings['la_lconic_layout_title']=='yes'){
					if(!empty($settings['title_text'])){
						$la_op .='<'.esc_attr($this->json_check_and_validate_html_tag($title_tag)).' class="jbafes-iconic-title">'.esc_attr($settings['title_text']).'</'.esc_attr($this->json_check_and_validate_html_tag($title_tag)).'>';
					}
				}
		...
		$la_op .='<div id="'.esc_attr($id).'" class="jbafes-lotties-animation-wrapper jbafes-'.esc_attr($id).' '.esc_attr($classes).'"  style="'.$style_atts.'" '.$kap_opt.'>';
		$la_op .='</div>';
